...
python3 new_experiment.py --visualize --vis-nodes 25 --vis-source 3 --vis-seed 46 --vis-range 200 --packets 20
...
class QMAR:
    ...
    def feedback(self, outcome, id_j, Q_value_best_action):
        alpha = self.drone.neighbor_table[id_j, 10] # learning rate
        gamma = self.drone.neighbor_table[id_j, 7]   # discount factor
        Q_value_i_j = self.drone.neighbor_table[id_j, 9] # the current Q value for using neighbor j to reach the destination
        #destination reached, incireaser Q value by alpha * maxReward
        if outcome == 1:
        ...
        # failure the packet was lost: apply penality reward - 5
        else:
            self.drone.neighbor_table[id_j, 9] = Q_value_i_j + alpha * (
    ...
    def relay_selection(self, opt_neighbors, data):
        packet = data[0] # packet to be forwarded
        candidates = []
        candidates2 = [] # fallback: positive but insufficient velocity

        # Debug info for any drone that runs relay_selection
        ...
        for node_j in self.simulator.drones:
            if node_j not in opt_neighbors:
                continue
            j = node_j.identifier
            # remaining time before the packet expires
            ...
            # dist_ij : the predicted distance between self and j at the time the packet arrives
        ...
        if not candidates:
            if candidates2:
                ...
                return "RHP"
        # choose next hop based on K-weighted Q value
        else:
                maxx = -100000
                chosen = None
    ...
    def computeActualVel(self, j, node_j, distance_i):
        x2 = self.drone.neighbor_table[j, 4] # current x
        y2 = self.drone.neighbor_table[j, 5] # current y
        x1 = self.drone.neighbor_table[j, 0] # previous x
        y1 = self.drone.neighbor_table[j, 1] # previous y
        angle_j = math.atan((y2 - y1) / (x2 - x1)) if (x2 - x1) != 0 else 0.0
        delay = self.drone.neighbor_table[j, 8] + self.drone.neighbor_table[j, 11]
        if delay == 0:
            delay = 0.01
        t1 = self.drone.neighbor_table[j, 6] # timestamp of last position update
        t3 = self.simulator.cur_step + delay # estimated time when packet arrives
        x = x1 + node_j.speed * math.cos(angle_j) * (t3 - t1) # future x
        y = y1 + node_j.speed * math.sin(angle_j) * (t3 - t1) # future y
        # predicted distances to the depot
...
def visualize_source_paths(G, station_id, src_id, comm_range, mode,
    ...
    G.nodes[src_id]['energy'] = src_drone.residual_energy # source starts with high energy so we can test dynamic w  
    qmar_dict = {}
